...
const createServer = () => {
  ...
  let game = app.listen(3301, function () {
    let port = game.address().port;
    console.log(port);
  });
  return game;
};
...
const currentEvents = {
  news: [
    ...
    {
      author: 'Authored by: Daniel Iglesias',
      category: [
        'technology',
        'gadgets'
      ],
...

    {
      ...
      description: 'Science can\'t decide how effective surgical masks and homemade alternatives are at preventing the spread of coronavirus. The CDC suggests making your own masks at home, but researchers can\'t say...',
...
function getCurrentEvents(request, response) {
  ...
  response.status(200).json(mapCurrentEvents());
}
...
const mapCurrentEvents = () => {
  ...
  let value = [];
  value = currentEvents.news.map(function (event) {
    let anEvent = new Event(event);
    return anEvent;
  });
  return value;
};
...
const countNumberOfElements = (arr) => arr.reduce((accumulator, val) => val, 0);
...
const returnNames = (arr) => {
  return arr.reduce((newArray, person) => {
...
const reversedString = (str) => {
  // Solution code here...
  let array = str.split('');
  return array.reduce( (string, letter) => {
